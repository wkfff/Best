...
uses Windows, SysUtils, Classes, Graphics, Forms, Controls, StdCtrls,
  ...
  cls_utils,dbtables,cls_obj,bstdb,adodb,AddFlow3Lib_TLB, gtQrCtrls,fonctions,
...
begin
  ...
  //    If  TDbRichEdit(sender as Tobject).Align = alClient then
        If Memo_width > 0 then
        begin
  //         TDbRichEdit(sender as Tobject).Align := alNone;
          ...
          Memo_Top   :=  0;
          Memo_Left  :=  0;
    {      If TDbRichEdit(sender as Tobject).Owner is Tfrm_ev then
           Begin
             (TDbRichEdit(sender as Tobject).Owner as Tform).WindowState := wsNormal;
              frm_ev.pgcp.Align      := alnone;

           end
          else
          If TDbRichEdit(sender as Tobject).Owner is Tfrm_regra then
           Begin
             (TDbRichEdit(sender as Tobject).Owner as Tform).WindowState := wsNormal;
             frm_regra.pgcp.Align      := alnone;
           end ; }
       ...
       begin
         ...
         { If TDbRichEdit(sender as Tobject).Owner is Tfrm_ev then
           Begin
             (TDbRichEdit(sender as Tobject).Owner as Tform).WindowState := wsMaximized;
             frm_ev.pgcp.Align      := alclient;
          ...
          If TDbRichEdit(sender as Tobject).Owner is Tfrm_regra then
           ...
           //  (TDbRichEdit(sender as Tobject).Owner as TForm).WindowState := wsMaximized;
           //  frm_regra.pgcp.Align      := alclient;
          { end ;

           Begin
             TDbRichEdit(sender as Tobject).Align := alClient;
             TDbRichEdit(sender as Tobject).BringToFront;
           end; }



          {TDbRichEdit(sender as Tobject).height:= self.height;
          TDbRichEdit(sender as Tobject).top   := self.top;
          TDbRichEdit(sender as Tobject).ScrollBars  := ssvertical;
          TDbRichEdit(sender as Tobject).BringToFront; }
...
begin
 ...
 // ver_obj.Text := frm_main.edit2.text
  //caption := caption + '                                 '+frm_main.edit2.text ;
  loadconstantObjs;
end;

...
uses
  ...
  IBCustomDataSet,Provider,DBClient, cls_bd ,ADODb,dbTables,math,strUtils, gtQrCtrls,fonctions,
  AddFlow3Lib_TLB  ,dbgrids;
...
begin
  ...
  sql.add(' CLI_ID = '+ ''+INTTOSTR(CLI_ID)+''+' AND ');
  sql.add(' PRJ_ID = '+ ''+INTTOSTR(PRJ_ID)+'');
  sql.add(' AND IDF_CODIGO IN ');
  sql.add(' ( ');
  sql.add(' SELECT FILHO_ID FROM COR_OBJETOS ');
  sql.add(' WHERE PAI_ID =  ');
  sql.add(' (  ');
  sql.add(' SELECT IDF_CODIGO FROM IDF_IF  ');
  sql.add(' WHERE IDF_DESIGNACAO = '''+ ptab + '''))');
  open;
  fclient.active := true;

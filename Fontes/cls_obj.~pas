...
begin
  if (MessageDlg(mens36,mtCustom, [mbYes, mbNo,mbCancel], 0) = mrYes) then
    self.Salvar;
end;
    ...
    Begin
        Begin
          wLock := tv.Locka_Diag(udado(tv.selected.data)^.Fcli,
                                 udado(tv.selected.data)^.FProj,
                                 udado(tv.selected.data)^.FDiag);
          If not wLock then  // encontrou diagrama bloqueado e nào conseguiu bloquear
            Begin
               Editavel         := false;
               odgLocked.MYLOCK := false;
               refresh;
...
//INICIA
        If not InList('CMB_DIVERSOS',itab) then
             begin
             with qry_cria_Idf_dados do
              begin
                close;
                ...
                If bdx.TIPOBANCO = 1 then
                   begin
                    sql.add('CREATE TABLE CMB_DIVERSOS( ');
                    sql.add('CLI_ID INTEGER NOT NULL ');
                    sql.Add('   ,DI_TIPO VARCHAR(3)');
                    SQL.ADD('   ,DI_DESCRICAO VARCHAR(25)');
                    SQL.ADD('   ,DI_LING VARCHAR(2)');
                    sql.add('        ,USUARIO VARCHAR(30)  ');
                    sql.add('       ,DATATU DATE         ');
                    execsql ;
                   end

...
uses
  ...
  DBCtrls,shellApi,Provider,ADODB,Fonctions,DB,bst_objetos,math, gtQrCtrls,printers
  ,gtQRRtns,bst_pativ,bst_pregra,bst_pdata,QRPrntr,gtqrXport_rtf,gtQRXport_DocumentDlg,
  gtQRXport_RTFDlg,cls_bd, FileCtrl, Animate, bst_dm_rel,GIFCtrl,bst_basefrm,
  ActnList, ToolWin;
...
type
  ...
  //  procedure Percorre(pNode: TTreeNode);
    ...
    procedure SetExportType(const Value: TgtExportType);
  ...
  published
   ...
   property ExportType: TgtExportType read FExportType write SetExportType;
...
begin
 ...
 If (frm_main.viewer) or (oAcesso.WACCESS = 1) then
   Begin
     ...
     entPop.Items[4].visible := false;
     speedbutton84.visible := false;
     sp_cop.visible       := false;
     xp_col.Visible        := false;
     sp_recort.Visible     := false;
     sss.Visible        := false;
...
procedure Tfrm_treeativ.SetExportType(const Value: TgtExportType);
begin
  FExportType := Value;
end;

...
uses
  ...
  RxGIF, Animate, GIFCtrl,IceLicense,reinit,idglobal,fonctions,DB_TIER,bst_BaseFrm,CL_ACCESS2,
  bst_dm_rel, ToolWin;
...
If not Checa_IceLicenca  then        //reativar
    Begin
      close;
      Application.terminate;
    end
...
begin
  {If not viewer then
    Begin
      If MessageDlg(mens150,mtConfirmation, [mbYes, mbNo], 0) = mrYes then
       Begin
...
begin
  try
   begin
     ...
     If bdx = nil then
      Raise exception.create(mens124) ;
     If tv <> nil then
       freeandnil(tv);
     If frm_logo <> nil then
      frm_logo.close;
...
begin
 { If wdemo then
   Begin
      with tEmpresa.create(self) do
        Begin
         If count_all = 0 then
          Begin
              wassist := true;
              If frm_demoassist     = nil then
                 frm_demoassist      := Tfrm_demoassist.create(self);
              frm_demoassist.showmodal;
          end;
    end;
   end
...
{
procedure Tfrm_main.demo_Assist;
 Begin
...
begin
  inherited;
  adotControleLicenca.ConnectionString := BDx.ConnectionString;
  adotControleLicenca.TableName := 'BST_VALIDACAO';
  adotControleLicenca.Active := true;
  QuantBanco := StrToInt(DesEncripta(adotControleLicenca.FieldByName('ATUACAO').AsString, 'Best71'));
  QuantBanco := QuantBanco + 1;
  adotControleLicenca.Edit;
  adotControleLicenca.FieldByName('ATUACAO').AsString := Encripta(IntToStr(QuantBanco), 'Best71');
  adotControleLicenca.Post;
  adotControleLicenca.Close;
end;

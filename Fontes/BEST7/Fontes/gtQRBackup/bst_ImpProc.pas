...
uses
  ...
  StdCtrls, ExtCtrls, FileCtrl, Buttons, gtQrCtrls,gtQRRtns, QuickRpt,cls_obj,
  Mask, Animate, GIFCtrl, ActnList, ImgList, ComCtrls, ToolWin;
...
procedure Tfrm_impproc.botClick(Sender: TObject);
  Begin
     ...
     Begin
      // frm_treeativ.Wimptxt := true ;
       ...
       case filt.itemindex of
          ...
          3:  Begin
                FRM_TREEATIV.ExportType := etXHTML;
                pArq := pArq + '.html';
              end;

          4:  Begin
                FRM_TREEATIV.ExportType := etText;
                pArq := pArq + '.txt';
              end;
...
procedure Tfrm_impproc.Image4Click(Sender: TObject);
begin
 botClick(nil);
end;
...
begin
  ...
  Label16.Caption          := defc31;//'Concluir'  ;
  speedButton5.Caption     := def901;// sar
end;

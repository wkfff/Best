...
uses
  ...
  dbctrls, bstdb, QuickRpt, Qrctrls, DB, cls_obj, ADODB;
...
If pqr is Tqrband then
 begin
   for i  := 0 to (pqr as Tqrband).ControlCount - 1 do
      begin
       If (pqr as Tqrband).controls[i] is TQRDBText then
          ((pqr as Tqrband).controls[i] as TQRDBText).dataset := pdataset
       else
       If (pqr as Tqrband).controls[i] is TQRDBRichText then
          ((pqr as Tqrband).controls[i] as TQRDBRichText).dataset := pdataset
       else   
...
If pqr is TqrChildband then
 begin
   for i  := 0 to (pqr as TqrChildband).ControlCount - 1 do
      begin
       If (pqr as TqrChildband).controls[i] is TQRDBText then
          ((pqr as TqrChildband).controls[i] as TQRDBText).dataset := pdataset
       else
       If (pqr as TqrChildband).controls[i] is TQRDBRichText then
          ((pqr as TqrChildband).controls[i] as TQRDBRichText).dataset := pdataset;
      end;
 end
else
If pqr is TqrSubDetail then
 begin
   ...
   for i  := 0 to (pqr as TqrSubDetail).ControlCount - 1 do
      begin
       If (pqr as TqrSubDetail).controls[i] is TQRDBText then
          ((pqr as TqrSubDetail).controls[i] as TQRDBText).dataset := pdataset
       else
       If (pqr as TqrSubDetail).controls[i] is TQRDBRichText then
          ((pqr as TqrSubDetail).controls[i] as TQRDBRichText).dataset := pdataset;
      end;
 end
else
 ...
 begin
  // (pqr as TqrGroup).DataSet := pDataset;
   for i  := 0 to (pqr as TqrGroup).ControlCount - 1 do
      begin
       If (pqr as TqrGroup).controls[i] is TQRDBText then
          ((pqr as TqrGroup).controls[i] as TQRDBText).dataset := pdataset
       else
       If (pqr as TqrGroup).controls[i] is TQRDBRichText then
          ((pqr as TqrGroup).controls[i] as TQRDBRichText).dataset := pdataset;
      end;
 end  ;
